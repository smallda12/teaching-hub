...
def main():
    ...
    群 = {}
    for x in sorted(單元們, key=lambda a: (序[a["領域"]], a["序"])):
        群.setdefault(x["領域"], []).append(x)
    資料 = [{"名": k, "圖": 圖[k], "色": 色[k],
            "單元": [{"序": u["序"], "名": u["單元"], "網址": u["網址"],
                    "週": u["週"], "頁": u["頁"], "題": u["題"], "卡": u["卡"]}
                   for u in v]}
          for k, v in sorted(群.items(), key=lambda a: 序[a[0]])]

    # 🛑 每個單元都要有素材數字；0 代表素材沒建置或路徑錯，不要靜默放行
    壞 = [u["名"] for d in 資料 for u in d["單元"]
         if not (u["頁"] and u["題"] and u["卡"])]
    ...
    for d in 資料:
        print("   %s %s　%d 單元" % (d["圖"], d["名"], len(d["單元"])))
